; (function () {
  ...
  function boot(selector) {
    ...
    if (el.nodeName == 'FORM') {
      el.$submit = el.querySelector('[type=submit]');
      el.$inputs = el.querySelectorAll('[data-rule]');
  ...
  function bindInputKeyup(inputEl){
    inputEl.addEventListener('keyup', e=>{
       inputEl.$errors = validateInput(inputEl);
      showInputErros(inputEl);
    })
  }
  ...
  function showInputErros(input){
    let errs = input.$errors;
    if(!errs.length){
  ...
  function validateInput(input) {
    ...
    let errors =input.$errors =  validate(inputValue, inputRule);
    return errors;
  }
